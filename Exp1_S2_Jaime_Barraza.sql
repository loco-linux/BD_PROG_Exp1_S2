...
-- FUNCION PARA CREAR EL CORREO DEL CLIENTE
CREATE OR REPLACE FUNCTION GENERAR_CORREO(
    P_APELLIDO VARCHAR2,
    P_EDAD NUMBER,
    P_NOMBRE VARCHAR2,
    P_FECHA_NAC DATE,
...
BEGIN
    RETURN 
        ...
        || '*' 
        || SUBSTR(P_NOMBRE, 1, 1) 
        || TO_CHAR(P_FECHA_NAC, 'DD')
        || SUBSTR(P_PERIODO, 2, 1) 
        || '@LogiCarg.cl';
END;
/


-- PARA ACTIVAR LA SALIDA EN LA CONSOLA
SET SERVEROUTPUT ON; 


-- VARIABLE BIND PARA ALMACENAR Y ASIGNAR EL PERIODO
...
DECLARE
    ...
    -- CREA UNA ESTRUCTURA IGUAL A LA TABLA COMO UN ARREGLO (ES DECIR, LOS CAMPOS, NO LOS VALORES)
	-- ME AHORRA ESCRIBIR MUCHAS VARIABLES

    V_PERIODO VARCHAR2(6) := :B_PERIODO; 
    V_TOTAL_CLIENTES NUMBER;            
    V_CONTADOR_CLIENTES NUMBER := 0;  


    -- NECESITO UN JOIN PARA OBTENER EL NOMBRE DE LA COMUNA
    -- VARIABLE QUE VOY A USAR PARA CALCULAR LA ASIGNACION DE PUNTAJES 
...
BEGIN
    DBMS_OUTPUT.PUT_LINE('EL PERIODO PROCESADO ES: ' || V_PERIODO);
    DBMS_OUTPUT.PUT_LINE('PROCESANDO CLIENTES ...');

    -- TRUNCAR LA TABLA DETALLE_DE_CLIENTES PARA ASEGURAR QUE NO HAYAN DATOS ANTIGUOS EN LA TABLA ANTES DE PROCESAR NUEVOS REGISTROS
    EXECUTE IMMEDIATE 'TRUNCATE TABLE DETALLE_DE_CLIENTES';

    -- OBTENGO EL TOTAL DE CLIENTES REALES PARA LUEGO COMPARARLOS CON LOS PROCESADOS AL FINAL DEL BUCLE
    SELECT COUNT(*) 
      INTO V_TOTAL_CLIENTES 
      FROM CLIENTE;


    -- BUCLE FOR (PARECIDO A UN FOR EACH) PARA PROCESAR A CADA CLIENTE
    FOR R_CLIENTE IN C_CLIENTES LOOP
        -- DATOS BASICOS DEL CLIENTE
        R_DETALLE.IDC     := R_CLIENTE.ID_CLI;
        R_DETALLE.RUT     := R_CLIENTE.NUMRUN_CLI;
        R_DETALLE.CLIENTE := INITCAP(R_CLIENTE.APPATERNO_CLI || ' ' || R_CLIENTE.APMATERNO_CLI || ' ' || R_CLIENTE.PNOMBRE_CLI);
        -- CALCULO DE LA EDAD, REDONDEADO, USANDO LA FECHA DE NACIMIENTO
        R_DETALLE.EDAD := FLOOR(MONTHS_BETWEEN(SYSDATE, R_CLIENTE.FECHA_NAC_CLI) / 12);

    -- REGLAS DE NEGOCIO PARA ASIGNAR PUNTAJES:
    -- SI RENTA > 700 MIL Y NO VIVE EN ESAS COMUNAS
    -- SE CALCULA EL PTJE REDONDEANDO EL 3% DE LA RENTA
    
        IF R_CLIENTE.RENTA > 700000 
           AND R_CLIENTE.NOMBRE_COMUNA NOT IN ('LA REINA', 'LAS CONDES', 'VITACURA') 
        THEN
            R_DETALLE.PUNTAJE := ROUND(R_CLIENTE.RENTA * 0.03);

        -- SE USO EL ID_TIPO_CLI PARA 'AHORRAR' UN JOIN Y ACCEDER AL NOMBRE DEL TIPO DE CLIENTE SIN ENTRAR EN LA TABLA 'TIPO_CLIENTE'
        -- SABIENDO QUE 'B' Y 'D' SON 'VIP' Y 'EXTRANJERO'
        -- EN ESTE CASO, EL PTJE ES 30 PTOS POR CADA AÑO DE EDAD QUE TENGA EL CLIENTE, SIEMPRE QUE CUMPLA LOS TIPOS B Y D.
        
        ELSIF R_CLIENTE.ID_TIPO_CLI IN ('B', 'D') THEN
            R_DETALLE.PUNTAJE := R_DETALLE.EDAD * 30;
        ELSE
        -- SI EL PUNTAJE ES CERO, SE CONSULTA LA TABLA TRAMO_EDAD PARA OBTENER EL PORCENTAJE DEL CLIENTE -SEGUN EDAD- Y APLICARLO A LA RENTA
            SELECT PORCENTAJE 
              INTO R_DETALLE.PUNTAJE
              FROM TRAMO_EDAD
             WHERE ANNO_VIG = EXTRACT(YEAR FROM SYSDATE)
               AND R_DETALLE.EDAD BETWEEN TRAMO_INF AND TRAMO_SUP;

            R_DETALLE.PUNTAJE := ROUND(R_DETALLE.PUNTAJE * R_CLIENTE.RENTA / 100);
        END IF;

        -- CONSTRUCCION DEL CORREO --> POR EJEMPLO: 'uval153*M183@LogiCarg.cl'
        R_DETALLE.CORREO_CORP := GENERAR_CORREO(R_CLIENTE.APPATERNO_CLI, R_DETALLE.EDAD, R_CLIENTE.PNOMBRE_CLI, R_CLIENTE.FECHA_NAC_CLI, V_PERIODO);


        R_DETALLE.PERIODO := SUBSTR(V_PERIODO, 1, 2) || '/' || SUBSTR(V_PERIODO, 3);

    -- INSERTA LOS VALORES DE LA ESTRUCTURA R_DETALLE A LA TABLA DETALLE_DE_CLIENTES AL FINAL DE CADA ITERACION
        INSERT INTO DETALLE_DE_CLIENTES VALUES R_DETALLE;
        
        -- SE INCREMENTA EN 1 EL CONTADOR POR CADA ITERACION DEL BUCLE
        V_CONTADOR_CLIENTES := V_CONTADOR_CLIENTES + 1;
    END LOOP;

    -- AL FINALIZAR EL BUCLE, COMPARA EL TOTAL DE CLIENTES ESPERADO CON RESPECTO A LA CANTIDAD DE CLIENTES QUE FUERON PROCESADOS EN EL BUCLE.
    -- SI SON IGUALES LOS VALORES, EL PROCESO FUE EXITOSO Y LO CONFIRMA CON UN COMMIT
    IF V_CONTADOR_CLIENTES = V_TOTAL_CLIENTES THEN
        DBMS_OUTPUT.PUT_LINE('Proceso Finalizado Exitosamente.');
        DBMS_OUTPUT.PUT_LINE('Se Procesaron : ' || V_CONTADOR_CLIENTES || ' CLIENTES');
        COMMIT;  
    -- DE LO CONTRARIO, MUESTRA UN MENSAJE DE ERROR Y HACE UN ROLLBACK PARA DESHACER LOS CAMBIOS.
    ELSE
...
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('[ERROR] SE PRODUJO UN ERROR: ' || SQLERRM);
        ROLLBACK;
END;
/

-- MUESTRA LA TABLA RESULTANTE, IGUAL AL FORMATO ESPERADO EN LA FIGURA 3
